...
impl method::GetAttr for RtObject {
    fn op_getattr(&self, rt: &Runtime, name: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_getattr, lhs, name)
    }

    fn native_getattr(&self, name: &Builtin) -> ObjectResult {
        native_foreach_builtin!(self.as_ref(), native_getattr, lhs, name)
    }
}
...
impl method::SetAttr for RtObject {
    fn op_setattr(&self, rt: &Runtime, name: &RtObject, value: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_setattr, lhs, name, value)
    }

    fn native_setattr(&self, name: &Builtin, value: &Builtin) -> RtResult<native::None> {
        native_foreach_builtin!(self.as_ref(), native_setattr, lhs, name, value)
    }
}
...
impl method::Id for RtObject {
    ...
    fn native_id(&self) -> native::ObjectId {
        expr_foreach_builtin!(self.as_ref(), obj, {
...
impl method::Hashed for RtObject {
    ...
    fn op_hash(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_hash, obj)
    }

    fn native_hash(&self) -> RtResult<native::HashId> {
        native_foreach_builtin!(self.as_ref(), native_hash, obj)
    }
}
...
impl method::StringCast for RtObject {
    fn op_str(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_str, obj)
    }

    fn native_str(&self) -> RtResult<native::String> {
        native_foreach_builtin!(self.as_ref(), native_str, obj)
    }
}
...
impl method::BytesCast for RtObject {
    fn op_bytes(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_bytes, obj)
    }

    fn native_bytes(&self) -> RtResult<native::Bytes> {
        native_foreach_builtin!(self.as_ref(), native_bytes, obj)
    }
}
...
impl method::StringFormat for RtObject {
    fn op_format(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_format, obj)
    }

    fn native_format(&self) -> RtResult<native::String> {
        native_foreach_builtin!(self.as_ref(), native_format, obj)
    }
}
...
impl method::StringRepresentation for RtObject {
    fn op_repr(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_repr, obj)
    }

    fn native_repr(&self) -> RtResult<native::String> {
        native_foreach_builtin!(self.as_ref(), native_repr, obj)
    }
}
...
impl method::Equal for RtObject {
    ...
    fn op_eq(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_eq, lhs, rhs)
    }

    /// Default implementation of equals fallsbacks to op_is.
    fn native_eq(&self, rhs: &Builtin) -> RtResult<native::Boolean> {
        native_foreach_builtin!(self.as_ref(), native_eq, lhs, rhs)
    }
}
...
impl method::NotEqual for RtObject {
    fn op_ne(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_ne, lhs, rhs)
    }

    fn native_ne(&self, rhs: &Builtin) -> RtResult<native::Boolean> {
        native_foreach_builtin!(self.as_ref(), native_ne, lhs, rhs)
    }
}

impl method::LessThan for RtObject {
    fn op_lt(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_lt, lhs, rhs)
    }

    fn native_lt(&self, rhs: &Builtin) -> RtResult<native::Boolean> {
        native_foreach_builtin!(self.as_ref(), native_lt, lhs, rhs)
    }
}
...
impl method::LessOrEqual for RtObject {
    fn op_le(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_le, lhs, rhs)
    }

    fn native_le(&self, rhs: &Builtin) -> RtResult<native::Boolean> {
        native_foreach_builtin!(self.as_ref(), native_le, lhs, rhs)
    }
}
...
impl method::GreaterOrEqual for RtObject {
    fn op_ge(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_ge, lhs, rhs)
    }

    fn native_ge(&self, rhs: &Builtin) -> RtResult<native::Boolean> {
        native_foreach_builtin!(self.as_ref(), native_ge, lhs, rhs)
    }
}
...
impl method::GreaterThan for RtObject {
    fn op_gt(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_gt, lhs, rhs)
    }

    fn native_gt(&self, rhs: &Builtin) -> RtResult<native::Boolean> {
        native_foreach_builtin!(self.as_ref(), native_gt, lhs, rhs)
    }
}
...
impl method::Index for RtObject {
    fn op_index(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_index, obj)
    }

    fn native_index(&self) -> RtResult<native::Integer> {
        native_foreach_builtin!(self.as_ref(), native_index, obj)
    }
}
...
impl method::NegateValue for RtObject {
    fn op_neg(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_neg, obj)
    }

    fn native_neg(&self) -> RtResult<native::Number> {
        native_foreach_builtin!(self.as_ref(), native_neg, obj)
    }
}
...
impl method::AbsValue for RtObject {
    fn op_abs(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_abs, obj)
    }

    fn native_abs(&self) -> RtResult<native::Number> {
        native_foreach_builtin!(self.as_ref(), native_abs, obj)
    }
}
...
impl method::PositiveValue for RtObject {
    fn op_pos(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_pos, obj)
    }

    fn native_pos(&self) -> RtResult<native::Number> {
        native_foreach_builtin!(self.as_ref(), native_pos, obj)
    }
}
...
impl method::InvertValue for RtObject {
    fn op_invert(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_invert, obj)
    }

    fn native_invert(&self) -> RtResult<native::Number> {
        native_foreach_builtin!(self.as_ref(), native_invert, obj)
    }
}
...
impl method::BitwiseAnd for RtObject {
    fn op_and(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_and, lhs, rhs)
    }

    fn native_and(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_and, lhs, rhs)
    }
}
...
impl method::DivMod for RtObject {
    fn op_divmod(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_divmod, lhs, rhs)
    }

    fn native_divmod(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_divmod, lhs, rhs)
    }
}
...
impl method::FloorDivision for RtObject {
    fn op_floordiv(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_floordiv, lhs, rhs)
    }

    fn native_floordiv(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_floordiv, lhs, rhs)
    }
}
...
impl method::LeftShift for RtObject {
    fn op_lshift(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_lshift, lhs, rhs)
    }

    fn native_lshift(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_lshift, lhs, rhs)
    }
}
...
impl method::Modulus for RtObject {
    fn op_mod(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_mod, lhs, rhs)
    }

    fn native_mod(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_mod, lhs, rhs)
    }
}
...
impl method::Multiply for RtObject {
    fn op_mul(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_mul, lhs, rhs)
    }

    fn native_mul(&self, rhs: &Builtin) -> RtResult<native::Native> {
        native_foreach_builtin!(self.as_ref(), native_mul, lhs, rhs)
    }
}
...
impl method::MatrixMultiply for RtObject {
    fn op_matmul(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_matmul, lhs, rhs)
    }

    fn native_matmul(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_matmul, lhs, rhs)
    }
}
...
impl method::BitwiseOr for RtObject {
    fn op_or(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_or, lhs, rhs)
    }

    fn native_or(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_or, lhs, rhs)
    }
}
...
impl method::Pow for RtObject {
    fn op_pow(&self, rt: &Runtime, power: &RtObject, modulus: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_pow, base, power, modulus)
    }

    fn native_pow(&self, power: &Builtin, modulus: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_pow, base, power, modulus)
    }
}
...
impl method::RightShift for RtObject {
    fn op_rshift(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_rshift, lhs, rhs)
    }

    fn native_rshift(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_rshift, lhs, rhs)
    }
}
...
impl method::Subtract for RtObject {
    fn op_sub(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_sub, lhs, rhs)
    }

    fn native_sub(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_sub, lhs, rhs)
    }
}
...
impl method::TrueDivision for RtObject {
    fn op_truediv(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_truediv, lhs, rhs)
    }

    fn native_truediv(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_truediv, lhs, rhs)
    }
}
...
impl method::XOr for RtObject {
    fn op_xor(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_xor, lhs, rhs)
    }

    fn native_xor(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_xor, lhs, rhs)
    }
}
...
impl method::InPlaceAdd for RtObject {
    fn op_iadd(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_iadd, lhs, rhs)
    }

    fn native_iadd(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_iadd, lhs, rhs)
    }
}
...
impl method::InPlaceBitwiseAnd for RtObject {
    fn op_iand(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_iand, lhs, rhs)
    }

    fn native_iand(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_iand, lhs, rhs)
    }
}
...
impl method::InPlaceDivMod for RtObject {
    fn op_idivmod(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_idivmod, lhs, rhs)
    }

    fn native_idivmod(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_idivmod, lhs, rhs)
    }
}
...
impl method::InPlaceFloorDivision for RtObject {
    fn op_ifloordiv(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_ifloordiv, lhs, rhs)
    }

    fn native_ifloordiv(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_ifloordiv, lhs, rhs)
    }
}
...
impl method::InPlaceLeftShift for RtObject {
    fn op_ilshift(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_ilshift, lhs, rhs)
    }

    fn native_ilshift(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_ilshift, lhs, rhs)
    }
}
...
impl method::InPlaceModulus for RtObject {
    fn op_imod(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_imod, lhs, rhs)
    }

    fn native_imod(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_imod, lhs, rhs)
    }
}
...
impl method::InPlaceMultiply for RtObject {
    fn op_imul(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_imul, lhs, rhs)
    }

    fn native_imul(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_imul, lhs, rhs)
    }
}
...
impl method::InPlaceMatrixMultiply for RtObject {
    fn op_imatmul(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_imatmul, lhs, rhs)
    }

    fn native_imatmul(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_imatmul, lhs, rhs)
    }
}
...
impl method::InPlaceBitwiseOr for RtObject {
    fn op_ior(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_ior, lhs, rhs)
    }

    fn native_ior(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_ior, lhs, rhs)
    }
}
...
impl method::InPlacePow for RtObject {
    fn op_ipow(&self, rt: &Runtime, power: &RtObject, modulus: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_ipow, base, power, modulus)
    }

    fn native_ipow(&self, power: &Builtin, modulus: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_ipow, base, power, modulus)
    }
}
...
impl method::InPlaceRightShift for RtObject {
    fn op_irshift(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_irshift, lhs, rhs)
    }

    fn native_irshift(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_irshift, lhs, rhs)
    }
}
...
impl method::InPlaceSubtract for RtObject {
    fn op_isub(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_isub, lhs, rhs)
    }

    fn native_isub(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_isub, lhs, rhs)
    }
}
...
impl method::InPlaceTrueDivision for RtObject {
    fn op_itruediv(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_itruediv, lhs, rhs)
    }

    fn native_itruediv(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_itruediv, lhs, rhs)
    }
}
...
impl method::InPlaceXOr for RtObject {
    fn op_ixor(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_ixor, lhs, rhs)
    }

    fn native_ixor(&self, rhs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_ixor, lhs, rhs)
    }
}
...
impl method::Contains for RtObject {
    fn op_contains(&self, rt: &Runtime, rhs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_contains, lhs, rhs)
    }

    fn native_contains(&self, rhs: &Builtin) -> RtResult<native::Boolean> {
        native_foreach_builtin!(self.as_ref(), native_contains, lhs, rhs)
    }
}
...
impl method::Iter for RtObject {
    fn op_iter(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_iter, lhs)
    }

    fn native_iter(&self) -> RtResult<native::Iterator> {
        native_foreach_builtin!(self.as_ref(), native_iter, lhs)
    }
}
...
impl method::Call for RtObject {
    fn op_call(&self, rt: &Runtime, pos_args: &RtObject, starargs: &RtObject, kwargs: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_call, method, pos_args, starargs, kwargs)
    }

    fn native_call(&self, pos_args: &Builtin, starargs: &Builtin, kwargs: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_call, method, pos_args, starargs, kwargs)
    }
}
...
impl method::Length for RtObject {
    fn op_len(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_len, lhs)
    }

    fn native_len(&self) -> RtResult<native::Integer> {
        native_foreach_builtin!(self.as_ref(), native_len, lhs)
    }
}
...
impl method::Next for RtObject {
    fn op_next(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, op_next, lhs)
    }

    fn native_next(&self) -> ObjectResult {
        native_foreach_builtin!(self.as_ref(), native_next, lhs)
    }
}
...
impl method::Append for RtObject {
    fn meth_append(&self, rt: &Runtime, name: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, meth_append, object, name)
    }

    fn native_meth_append(&self, name: &Builtin) -> RtResult<native::None> {
        native_foreach_builtin!(self.as_ref(), native_meth_append, object, name)
    }
}
...
impl method::Extend for RtObject {
    fn meth_extend(&self, rt: &Runtime, name: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, meth_extend, object, name)
    }

    fn native_meth_extend(&self, name: &Builtin) -> RtResult<native::None> {
        native_foreach_builtin!(self.as_ref(), native_meth_extend, object, name)
    }
}
...
impl method::Pop for RtObject {
    fn meth_pop(&self, rt: &Runtime, name: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, meth_pop, object, name)
    }

    fn native_meth_pop(&self, name: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_meth_pop, object, name)
    }
}
...
impl method::Remove for RtObject {
    fn meth_remove(&self, rt: &Runtime, name: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, meth_remove, object, name)
    }

    fn native_meth_remove(&self, name: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_meth_remove, object, name)
    }
}
...
impl method::IsDisjoint for RtObject {
    fn meth_isdisjoint(&self, rt: &Runtime, name: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, meth_isdisjoint, object, name)
    }

    fn native_meth_isdisjoint(&self, name: &Builtin) -> RtResult<native::Boolean> {
        native_foreach_builtin!(self.as_ref(), native_meth_isdisjoint, object, name)
    }
}
...
impl method::AddItem for RtObject {
    fn meth_add(&self, rt: &Runtime, name: &RtObject) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, meth_add, object, name)
    }

    fn native_meth_add(&self, name: &Builtin) -> RtResult<Builtin> {
        native_foreach_builtin!(self.as_ref(), native_meth_add, object, name)
    }
}
...
impl method::Keys for RtObject {
    fn meth_keys(&self, rt: &Runtime) -> ObjectResult {
        foreach_builtin!(self.as_ref(), rt, meth_keys, object)
    }

    fn native_meth_keys(&self) -> RtResult<native::Tuple> {
        native_foreach_builtin!(self.as_ref(), native_meth_keys, object)
    }
}
