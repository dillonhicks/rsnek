...
use token::Tk;
pub type TkBuf<'a> = [Tk<'a>];

#[derive(Clone,Copy)]
#[repr(C)]
pub struct TkSlice<'a> {
  buf: &'a TkBuf<'a>,
  start: usize,
  end:   usize,
}

impl<'a> fmt::Debug for TkSlice<'a> {
  fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
    write!(f, "TkSlice {{ start: {}, end: {}, data:\n{}\n}}", self.start, self.end, self.buf.bytes().to_hex(16))
  }
}

impl<'a> PartialEq for TkSlice<'a> {
  fn eq(&self, other: &TkSlice<'a>) -> bool {
...
impl<'a> InputIter for TkSlice<'a> {
    ...
    fn position<P>(&self, predicate: P) -> Option<usize> where P: Fn(Self::RawItem) -> bool {
      self.buf.iter().position(|b| predicate(b))
    }
...
impl<'a,'b> Compare<&'b[u8]> for TkSlice<'a> {
...
impl<'a,'b> Compare<&'b str> for TkSlice<'a> {
...
fn sl<'a>(input: &'a BlockBuf) -> TkSlice<'a> {
  TkSlice {
    buf: input,
    start: 0,
    end:   input.len(),
...
fn to_i64<'a>(input: TkSlice<'a>) -> Result<i64, ()> {
