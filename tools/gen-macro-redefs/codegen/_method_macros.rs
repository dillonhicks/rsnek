...
//!
//! ```ignore
//! _method!(my_function<Parser<'a> >, self, tag!("abcd"));
//! ```
//!
...
//! ```
//!
//! The `_method!` macro is similar to the `_named!` macro in the macros module.
//! While `named!` will create a parser function, `_method!` will create a parser
//! method on the struct it is defined in.
//!
//! Compared to the `_named!` macro there are a few differences in how they are
//! invoked. A `_method!` invocation always has to have the type of `self`
//! declared and it can't be a reference due to Rust's borrow lifetime
//! restrictions:
//! ```ignore
//! //                  -`self`'s type-
//! _method!(method_name<  Parser<'a> >, ...);
//! ```
...
//! ```ignore
//! //                                                 -self-
//! _method!(method_name<Parser<'a>, &'a str, &'a str>, self, ...);
//! ```
...
//!   _method!(take4<Parser<'a>, &'a str, &'a str>, self, take!(4));
//!   _method!(caller<Parser<'a>, &'a str, &'a str>, self, call_m!(self.take4));
//! }
//! ```
//! More complicated combinations still mostly look the same as their `_named!`
//! counterparts:
//! ```ignore
//!    _method!(pub simple_chain<&mut Parser<'a>, &'a str, &'a str>, self,
//!      do_parse!(
//!             call_m!(self.tag_abc)                                        >>
//!             call_m!(self.tag_def)                                        >>
//!             call_m!(self.tag_ghi)                                        >>
...
macro_rules! _method (
  ...
  ($name:ident<$a:ty,$o:ty>, $self_:ident, $submac:ident!( $($args:tt)* )) => (
      #[allow(unused_variables)]
      fn $name( $self_: $a, i:  ) -> ($a, $nom::IResult<, $o, u32>) {
        let result = $submac!(i, $($args)*);
        ($self_, result)
      }
  );
  ($name:ident<$a:ty>, $self_:ident, $submac:ident!( $($args:tt)* )) => (
      #[allow(unused_variables)]
      fn $name( $self_: $a, i:  ) -> ($a, $nom::IResult<, , u32>) {
        let result = $submac!(i, $($args)*);
        ($self_, result)
      }
  );
  ...
  (pub $name:ident<$a:ty,$o:ty>, $self_:ident, $submac:ident!( $($args:tt)* )) => (
    #[allow(unused_variables)]
    pub fn $name( $self_: $a, i:  ) -> ($a, $nom::IResult<, $o, u32>) {
      let result = $submac!(i, $($args)*);
      ($self_, result)
    }
  );
  (pub $name:ident<$a:ty>, $self_:ident, $submac:ident!( $($args:tt)* )) => (
    #[allow(unused_variables)]
    pub fn $name( $self_: $a, i:  ) -> ($a, $nom::IResult<, , u32>) {
      let result = $submac!(i, $($args)*);
      ($self_, result)
    }
  );
  ...
  ($name:ident<$a:ty,$o:ty>, mut $self_:ident, $submac:ident!( $($args:tt)* )) => (
      #[allow(unused_variables)]
      fn $name( mut $self_: $a, i:  ) -> ($a, $nom::IResult<, $o, u32>) {
        let result = $submac!(i, $($args)*);
        ($self_, result)
      }
  );
  ($name:ident<$a:ty>, mut $self_:ident, $submac:ident!( $($args:tt)* )) => (
      #[allow(unused_variables)]
      fn $name( mut $self_: $a, i:  ) -> ($a, $nom::IResult<, , u32>) {
        let result = $submac!(i, $($args)*);
        ($self_, result)
      }
  );
  ...
  (pub $name:ident<$a:ty,$o:ty>, mut $self_:ident, $submac:ident!( $($args:tt)* )) => (
    #[allow(unused_variables)]
    pub fn $name( mut $self_: $a, i:  ) -> ($a, $nom::IResult<, $o, u32>) {
      let result = $submac!(i, $($args)*);
      ($self_, result)
    }
  );
  (pub $name:ident<$a:ty>, mut $self_:ident, $submac:ident!( $($args:tt)* )) => (
    #[allow(unused_variables)]
    pub fn $name( mut $self_: $a, i:  ) -> ($a, $nom::IResult<, , u32>) {
      let result = $submac!(i, $($args)*);
      ($self_, result)
    }
  );
